 ...
 * 처음에 그는 농구 경기를 좋아하는 사람이었다. 농구에 대한 열정은 그를 막을 수 없었고, 결국 상근이는 농구장을 청소하는 일을 시작했다. 
 * 상근이도 농구장을 청소하면서 감독이 되기 위해 가져야할 능력을 공부해나갔다. 서당개 3년이면 풍월을 읊듯이 상근이는 점점 감독으로 한 걸음 다가가고 있었다. 
...
public class No1159_BasketBall {
	public static void main(String[] args) {
		...
		int n = sc.nextInt();
		Map<String, Integer> names = new TreeMap<String, Integer>(); // 사전순 정렬을 위한 TreeMap 선언
		
		for (int i = 0; i < n; i++) {
			String name = sc.next();
			names.put(name.substring(0, 1), names.getOrDefault(name.substring(0, 1), 0) + 1); // 선수들의 성 분포도 map에 저장
		}
		sc.close();
